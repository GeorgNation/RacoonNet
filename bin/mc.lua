...
local config = racoon.readconfig("mc")
local locale = racoon.readlang("mc")
local theme
...
local function eventTouch(tx,ty,mTouch)
  ...
  elseif ty>1 and ty<hScr-2 then
    if ty-1 == Active.CurLine and (code==0 or code==Ctrl) and
     not panChang and lastClick >= pc.uptime()-0.5 then
